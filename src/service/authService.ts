...
export async function verifyOtpService(otpCode: number) {
  ...
  const response = await fetch(
    `http://localhost:8080/api/v1/auth/verify-otp?otpCode=${otpCode}`,
    {
      method: "PUT",
      headers: headers,
    }
...
export async function resendOtpService(email: string) {
  ...
  const response = await fetch(
    `http://localhost:8080/api/v1/auth/resend-otp?email=${email}`,
    {
      method: "PUT",
      headers: headers,
    }
...
export async function changePasswordService(email: string) {
  const { headers } = await headerToken(false);
  const response = await fetch(
    `http://localhost:8080/api/v1/auth/change-password?email=${email}`,
    {
      method: "PUT",
      headers: headers,
    }
...
export async function getUserDetailService(email: string) {
  const { headers } = await headerToken(false);
  const response = await fetch(
    `http://localhost:8080/api/v1/auth/user-detail?email=${email}`,
    {
      method: "GET",
      headers: headers,
    }
...
export async function recoveryAccountService(email: string) {
  const { headers } = await headerToken(false);
  const response = await fetch(
    `http://localhost:8080/api/v1/auth/recovery-password?email=${email}`,
    {
      method: "GET",
      headers: headers,
    }
